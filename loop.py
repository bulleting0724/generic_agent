"""Generic AgentLoop: domain-neutral ReAct core loop.
...
Five-layer context management:
  ...
  Layer 5 (iterative update) — Nth compression updates previous summary instead of fresh

Tool execution:
  - Read/write batching: consecutive readonly tools run in parallel via threads

Usage:
    from generic_agent import AgentLoop, ContextBuilder, LLMClient, ToolRegistry
    ...
    context = ContextBuilder(registry, memory, system_prompt_template=MY_PROMPT)
    loop = AgentLoop(registry=registry, llm=MyLLM(), context_builder=context)
    result = loop.run("user message")
"""
...
from progress import HeartbeatTimer, ProgressEvent, _set_emitter
...
class LLMError(Exception):
    """Exception raised by LLM client errors.

    Attributes:
        retryable: Whether the operation can be retried.
        provider: Provider name (for diagnostics).
        model: Model name (for diagnostics).
    """
...
@dataclass
class ToolCall:
    """Represents a tool call requested by the LLM response."""
...
@dataclass
class LLMResponse:
    """Represents an LLM response (streamed or non-streamed).

    Attributes:
        content: Text content of the response.
        tool_calls: Tool calls requested by the model.
        usage_metadata: Token usage info from the provider.
        content_filter_triggered: Whether content moderation blocked the response.
        reasoning_content: Provider-specific reasoning/thinking text.
    """
...
class LLMClient(ABC):
    """Abstract LLM client for agent inference.

    Implementations must provide ``stream_chat`` (streaming with callbacks)
    and ``chat`` (non-streaming). The ``model_name`` attribute is used for
    diagnostics / usage tracking.
    """
    ...
    def stream_chat(
        ...
        Args:
            ...
            on_text_chunk: Called with each text delta.
            on_reasoning_chunk: Called with each reasoning delta.
            should_cancel: Pollable cancel flag.

        Returns:
            An LLMResponse with content, tool_calls, etc.
        """
...
class RunStateStore:
    ...
    def save_request(
        self, run_dir: Path, prompt: str, context: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def mark_success(self, run_dir: Path) -> None:
        ...
        """
        self._write_json(
            run_dir / "state.json", {"status": "success"}
        )
...
def compute_content_filter_warnings(
    ...
    try:
        ...
        threshold = DEFAULT_CONTENT_FILTER_THRESHOLD
    if ratio > threshold:
        return [
...
HEARTBEAT_INTERVAL_S = float(os.getenv("AGENT_HEARTBEAT_INTERVAL_S", "3.0"))
REASONING_DELTA_MIN_INTERVAL_S = float(
    os.getenv("AGENT_REASONING_DELTA_MIN_INTERVAL_S", "1.0")
)
STREAM_RETRY_DELAY_S = float(os.getenv("AGENT_STREAM_RETRY_DELAY_S", "1.0"))
TOOL_TIMEOUT_SECONDS = float(
    os.getenv("AGENT_TOOL_TIMEOUT_SECONDS", "1800")
)
...
def _new_llm_usage_summary(llm: Any) -> dict[str, Any]:
    ...
    provider = (
        os.getenv("LANGCHAIN_PROVIDER", "openai").strip() or "openai"
    )
    model = (
...
def _record_llm_usage(
    ...
    summary.setdefault("per_iteration", []).append(
        {"iter": iteration, **normalized}
    )
    summary["updated_at"] = (
...
def _redact_trace_result(result: str) -> str:
    """Pass-through: trace results are stored as-is.

    Override or monkey-patch this function to add custom redaction logic
    (e.g. for credentials or PII) without changing the framework code.
    """
    return result
...
def estimate_tokens(messages: list) -> int:
    ...
    return (
        len(json.dumps(messages, default=str, ensure_ascii=False)) // 4
    )
...
def _fix_tool_pairs(messages: list) -> None:
    ...
    while i < len(messages):
        ...
        if (
            msg.get("role") == "tool"
            and msg.get("tool_call_id") not in call_ids
        ):
            messages.pop(i)
        else:
            i += 1
...
User-stated requirements: constraints, preferences, parameter values.
...
def _normalize_tool_run_dir(
    ...
    if not candidate.is_absolute():
        normalized["run_dir"] = str(
            (Path(memory_run_dir) / candidate).resolve()
        )
    return normalized


class AgentLoop:
    """ReAct Agent core loop — domain neutral.
    ...
    def __init__(
        ...
        Args:
            ...
            context_builder: Pre-configured ContextBuilder.  If not provided
                one is created from ``registry`` and ``memory`` with the
    ...
    def cancel(self) -> None:
        ...
        stream chunk, and between tool batches.
        """
    ...
    def run(
        ...
        state_store.save_request(
            run_dir, user_message, {"session_id": session_id}
        )
        ...
        trace_dir = (
            SESSIONS_DIR / session_id if session_id else run_dir
        )
        ...
        try:
            while iteration < self.max_iterations:
                if self._cancel_event.is_set():
                    trace.write(
                        {"type": "cancelled", "iter": self._run_iteration + 1}
                    )
                ...
                if tokens > TOKEN_THRESHOLD:
                    logger.info(
                        f"Auto compact triggered: {tokens} tokens > {TOKEN_THRESHOLD}"
                    )
                    self._auto_compact(
                ...
                # Wrap-up nudge
                if (
                    iteration == wrap_up_at
                    and 1 < iteration < self.max_iterations
                ):
                ...
                def _on_text_chunk(delta: str) -> None:
                    ...
                    self._emit(
                        "text_delta", {"delta": delta, "iter": current_iter}
                    )
                ...
                def _on_reasoning_chunk(delta: str) -> None:
                    ...
                    if (
                        ...
                        < REASONING_DELTA_MIN_INTERVAL_S
                    ):
                        return
                ...
                if is_last_iteration:
                    trace.write(
                        {"type": "forced_text_only", "iter": current_iter}
                    )
        ...
        if self._cancel_event.is_set():
            ...
            provider = (
                os.getenv("LANGCHAIN_PROVIDER", "openai").strip().lower()
                or "openai"
            )
            model = (
            ...
            final_reason = (
                ...
                f"iteration {empty_model_response_iter} "
    ...
    def _process_tool_calls(
        ...
        for tc in tool_calls:
            ...
            if tc.name == "compact":
                ...
                messages.append(
                    context.format_tool_result(
                        tc.id,
                        "compact",
                        '{"status":"ok","message":"Compressing..."}',
                    )
                )
                trace.write(
                    {"type": "compact_requested", "iter": iteration}
                )
                continue
            ...
            if tc.name in self._called_ok and not is_repeatable:
                ...
                react_trace.append(
                    {"type": "tool_skipped", "tool": tc.name}
                )
                continue
    ...
    def _execute_parallel(
        ...
        for tc in tool_calls:
            args = _normalize_tool_run_dir(
                tc.arguments, self.memory.run_dir
            )
            redacted_args = args
            event_args = {
                k: str(v)[:200] for k, v in redacted_args.items()
            }
            self._emit(
                "tool_call",
                {
    ...
    def _invoke_tool(
        ...
        timeout = (
            TOOL_TIMEOUT_SECONDS if TOOL_TIMEOUT_SECONDS > 0 else None
        )
        timeout_label = (
            _format_timeout(timeout) if timeout is not None else ""
        )
        ...
        def _emit_timeout_progress(
            stage: str, message: str, **extra: Any
        ) -> int:
        ...
        def _worker() -> None:
            ...
            try:
                result_queue.put(
                    (self.registry.execute(tool_name, args), None)
                )
        ...
        with _heartbeat_timer():
            try:
                ...
                elapsed_ms = _emit_timeout_progress(
                    "timeout",
                    f"Tool exceeded {timeout_label} timeout",
                )
                return (
                    json.dumps(
                        {
    ...
    def _finalize_tool_result(
        ...
        messages.append(
            context.format_tool_result(tc.id, tc.name, truncated)
        )
    ...
    def _auto_compact(
        ...
        Upgrades over the original:
          - Token-budget tail: keeps ~20K tokens of recent messages.
          - Structured summary template: preserves goal, progress, decisions, etc.
          - Iterative update: Nth compression updates previous summary.
        ...
        transcript_path = (
            trace.dir_path
            / f"transcript_{int(_time.time())}.jsonl"
        )
        ...
        while (
            0 < cut_idx < len(body)
            and body[cut_idx].get("role") == "tool"
        ):
            cut_idx += 1
        ...
        if not head:
            if len(body) > 2:
                ...
                logger.warning(
                    "Auto compact: nothing to compress (body too small)"
                )
                return

        focus_section = (
            _FOCUS_SECTION.format(topic=focus_topic) if focus_topic else ""
        )

        conv_text = json.dumps(head, default=str, ensure_ascii=False)[
            :80000
        ]
        ...
        compressed = (
            f"[Conversation compressed — handoff summary. "
