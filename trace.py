...
Self-contained copy (no Vibe-Trading dependency).
"""
...
TOOL_RESULT_OFFLOAD_THRESHOLD = _env_int(
    "AGENT_TRACE_TOOL_RESULT_INLINE_LIMIT",
    50_000,
)
TRACE_TEXT_OFFLOAD_THRESHOLD = _env_int(
    "AGENT_TRACE_TEXT_INLINE_LIMIT",
    50_000,
)
OFFLOAD_PREVIEW_CHARS = _env_int("AGENT_TRACE_PREVIEW_CHARS", 500)
...
class TraceWriter:
    ...
    def write_text_entry(
        ...
        *,
        ...
        Args:
            ...
            offload_kind: Stable label used in sidecar filename hash input.
            threshold: Optional inline threshold override (default from env).
        """
    ...
    def read(
        ...
        *,
        ...
        Args:
            ...
            resolve_fields: Optional field allowlist to resolve.
        ...
        if not path.exists():
            return []
        fields = (
            set(resolve_fields) if resolve_fields is not None else None
        )
        ...
        for line in path.read_text(encoding="utf-8").splitlines():
            ...
            try:
                ...
                continue
            if resolve_offloads:
                TraceWriter._resolve_entry_offloads(
                    dir_path, entry, fields
                )
    ...
    def _resolve_entry_offloads(
        ...
        for key, rel_path in list(entry.items()):
            ...
            if field in entry:
                continue
            result_file = TraceWriter._safe_sidecar_path(
                dir_path, rel_path
            )
