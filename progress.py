...
Two mechanisms:
  * Tool-level heartbeat: a background timer fires keepalive events every N
    seconds while a tool runs, so the UI never looks frozen.
  * Structured progress: tools can opt in via ``emit_progress()`` to publish
    quantified state (stage, current/total, message).
...
def emit_progress(
    ...
    *,
    ...
    Silently no-ops when called outside an active tool context. Never raises.
